object DM: TDM
  Height = 750
  Width = 1000
  PixelsPerInch = 120
  object RL_Departamentos: TfrxReport
    Version = '2024.2.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick, pbCopy, pbSelection]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 45553.615466273100000000
    ReportOptions.LastChange = 45553.622222337970000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 32
    Top = 32
    Datasets = <
      item
        DataSet = RLDS_Departamentos
        DataSetName = 'frxDBDepartamentos'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 109.606370000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Left = 109.606369020000000000
          Top = 18.897649530000000000
          Width = 449.764051550000000000
          Height = 45.354363730000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -35
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'Relat'#243'rio de Departamentos')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 120.944960000000000000
        Top = 188.976500000000000000
        Width = 718.110700000000000000
        DataSet = RLDS_Departamentos
        DataSetName = 'frxDBDepartamentos'
        RowCount = 0
        object frxDBDepartamentosid_departamento: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 143.622140000000000000
          Top = 18.897650000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'id_departamento'
          DataSet = RLDS_Departamentos
          DataSetName = 'frxDBDepartamentos'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDepartamentos."id_departamento"]')
        end
        object frxDBDepartamentosnm_departamento: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 317.480520000000000000
          Top = 18.897650000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'nm_departamento'
          DataSet = RLDS_Departamentos
          DataSetName = 'frxDBDepartamentos'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDepartamentos."nm_departamento"]')
        end
        object frxDBDepartamentoslocal: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 64.252010000000000000
          Top = 52.913420000000000000
          Width = 506.457020000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'local'
          DataSet = RLDS_Departamentos
          DataSetName = 'frxDBDepartamentos'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDepartamentos."local"]')
        end
        object Memo2: TfrxMemoView
          AllowVectorExport = True
          Left = 15.118118240000000000
          Top = 18.897645510000000000
          Width = 120.944968360000000000
          Height = 18.897659300000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'ID Departamento:')
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          AllowVectorExport = True
          Left = 268.346630000000000000
          Top = 18.897650000000000000
          Width = 56.692958360000000000
          Height = 18.897659300000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Nome:')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Left = 15.118120000000000000
          Top = 52.913420000000000000
          Width = 45.354368360000000000
          Height = 18.897659300000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Local:')
          ParentFont = False
        end
        object Line1: TfrxLineView
          AllowVectorExport = True
          Left = -0.000000230000000000
          Top = 98.267793210000000000
          Width = 782.362726210000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
    end
  end
  object RLDS_Empregados: TfrxDBDataset
    UserName = 'frxDBDatasetEmpregados'
    CloseDataSource = False
    DataSet = ZQ_RptEmpregados
    BCDToCurrency = False
    DataSetOptions = []
    Left = 48
    Top = 112
  end
  object RLDS_Departamentos: TfrxDBDataset
    UserName = 'frxDBDepartamentos'
    CloseDataSource = False
    DataSet = ZQ_RptDepartamentos
    BCDToCurrency = False
    DataSetOptions = []
    Left = 240
    Top = 112
  end
  object ZConnection: TZConnection
    ControlsCodePage = cCP_UTF16
    Catalog = ''
    Properties.Strings = (
      'RawStringEncoding=DB_CP')
    Connected = True
    DisableSavepoints = False
    HostName = 'localhost'
    Port = 5432
    Database = 'postgres'
    User = 'postgres'
    Password = '1501933'
    Protocol = 'postgresql'
    Left = 912
    Top = 32
  end
  object ZQ_Empregados: TZQuery
    Connection = ZConnection
    BeforePost = ZQ_EmpregadosBeforePost
    Active = True
    SQL.Strings = (
      'select * from empregados')
    Params = <>
    Left = 760
    Top = 136
    object ZQ_Empregadosid_empregado: TZIntegerField
      FieldName = 'id_empregado'
      Required = True
    end
    object ZQ_Empregadoscod_departamento: TZIntegerField
      FieldName = 'cod_departamento'
      Required = True
    end
    object ZQ_Empregadosnm_empregado: TZUnicodeStringField
      FieldName = 'nm_empregado'
      Size = 100
    end
    object ZQ_Empregadosnm_funcao: TZUnicodeStringField
      FieldName = 'nm_funcao'
      Size = 100
    end
    object ZQ_Empregadosdata_admissao: TZDateField
      FieldName = 'data_admissao'
      OnSetText = ZQ_Empregadosdata_admissaoSetText
      DisplayFormat.Format = 'dd/mm/yyyy'
      EditMask = '!99/99/0000;1;_'
    end
    object ZQ_Empregadossalario: TZFMTBCDField
      FieldName = 'salario'
      DisplayFormat = '#,##0.00'
      Size = 0
    end
    object ZQ_Empregadoscomissao: TZFMTBCDField
      FieldName = 'comissao'
      DisplayFormat = '#,##0.00'
      Size = 0
    end
  end
  object DS_Empregados: TDataSource
    DataSet = ZQ_Empregados
    Left = 584
    Top = 136
  end
  object DS_Departamentos: TDataSource
    DataSet = ZQ_Departamentos
    Left = 584
    Top = 32
  end
  object ZQ_Departamentos: TZQuery
    Connection = ZConnection
    BeforePost = ZQ_DepartamentosBeforePost
    Active = True
    SQL.Strings = (
      'select * from departamentos')
    Params = <>
    Left = 752
    Top = 32
    object ZQ_Departamentosid_departamento: TZIntegerField
      FieldName = 'id_departamento'
    end
    object ZQ_Departamentosnm_departamento: TZUnicodeStringField
      FieldName = 'nm_departamento'
      Required = True
      Size = 100
    end
    object ZQ_Departamentoslocal: TZUnicodeStringField
      FieldName = 'local'
      Size = 100
    end
  end
  object ZQ_RptDepartamentos: TZQuery
    Connection = ZConnection
    BeforePost = ZQ_DepartamentosBeforePost
    Active = True
    SQL.Strings = (
      'select * from departamentos')
    Params = <>
    Left = 192
    Top = 240
    object ZIntegerField1: TZIntegerField
      FieldName = 'id_departamento'
    end
    object ZUnicodeStringField1: TZUnicodeStringField
      FieldName = 'nm_departamento'
      Required = True
      Size = 100
    end
    object ZUnicodeStringField2: TZUnicodeStringField
      FieldName = 'local'
      Size = 100
    end
  end
  object ZQ_RptEmpregados: TZQuery
    Connection = ZConnection
    BeforePost = ZQ_EmpregadosBeforePost
    Active = True
    SQL.Strings = (
      'select * from empregados')
    Params = <>
    Left = 200
    Top = 344
    object ZIntegerField2: TZIntegerField
      FieldName = 'id_empregado'
      Required = True
    end
    object ZIntegerField3: TZIntegerField
      FieldName = 'cod_departamento'
      Required = True
    end
    object ZUnicodeStringField3: TZUnicodeStringField
      FieldName = 'nm_empregado'
      Size = 100
    end
    object ZUnicodeStringField4: TZUnicodeStringField
      FieldName = 'nm_funcao'
      Size = 100
    end
    object ZDateField1: TZDateField
      FieldName = 'data_admissao'
      OnSetText = ZQ_Empregadosdata_admissaoSetText
      DisplayFormat.Format = 'dd/mm/yyyy'
      EditMask = '!99/99/0000;1;_'
    end
    object ZFMTBCDField1: TZFMTBCDField
      FieldName = 'salario'
      DisplayFormat = '#,##0.00'
      Size = 0
    end
    object ZFMTBCDField2: TZFMTBCDField
      FieldName = 'comissao'
      DisplayFormat = '#,##0.00'
      Size = 0
    end
  end
  object RL_Empregados: TfrxReport
    Version = '2024.2.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick, pbCopy, pbSelection]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 45553.580180069400000000
    ReportOptions.LastChange = 45562.644989363430000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 160
    Top = 32
    Datasets = <
      item
        DataSet = RLDS_Empregados
        DataSetName = 'frxDBDatasetEmpregados'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 162.519784440000000000
        Top = 158.740260000000000000
        Width = 718.110700000000000000
        DataSet = RLDS_Empregados
        DataSetName = 'frxDBDatasetEmpregados'
        RowCount = 0
        object LBL_IDEmpregado: TfrxMemoView
          AllowVectorExport = True
          Left = 7.559059960000000000
          Top = 15.118122400000000000
          Width = 105.826835380000000000
          Height = 22.677174820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'ID Empregado:')
          ParentFont = False
        end
        object LBL_CodDepartamento: TfrxMemoView
          AllowVectorExport = True
          Left = 211.653680000000000000
          Top = 15.118120000000000000
          Width = 147.401665380000000000
          Height = 22.677174820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'C'#243'digo Departamento:')
          ParentFont = False
        end
        object LBL_Nome: TfrxMemoView
          AllowVectorExport = True
          Left = 7.559060000000000000
          Top = 56.692950000000000000
          Width = 56.692945380000000000
          Height = 22.677174820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Nome:')
          ParentFont = False
        end
        object LBL_Funcao: TfrxMemoView
          AllowVectorExport = True
          Left = 192.756030000000000000
          Top = 56.692950000000000000
          Width = 56.692945380000000000
          Height = 22.677174820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Fun'#231#227'o:')
          ParentFont = False
        end
        object divider: TfrxMemoView
          AllowVectorExport = True
          Left = 7.559060000000000000
          Top = 98.267770280000000000
          Width = 952.441537870000000000
          Height = 64.252014160000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Frame.Typ = []
          Memo.UTF8W = (
            
              '________________________________________________________________' +
              '______________________________')
        end
        object LBL_DataAdmissao: TfrxMemoView
          AllowVectorExport = True
          Left = 461.102660000000000000
          Top = 15.118120000000000000
          Width = 136.063075380000000000
          Height = 22.677174820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Data de Admiss'#227'o:')
          ParentFont = False
        end
        object LBL_Salario: TfrxMemoView
          AllowVectorExport = True
          Left = 419.527830000000000000
          Top = 56.692950000000000000
          Width = 60.472475380000000000
          Height = 22.677174820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Sal'#225'rio:')
          ParentFont = False
        end
        object LBL_Comissao: TfrxMemoView
          AllowVectorExport = True
          Left = 551.811380000000000000
          Top = 56.692950000000000000
          Width = 75.590595380000000000
          Height = 22.677174820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Comiss'#227'o:')
          ParentFont = False
        end
        object frxDBDatasetEmpregadosid_empregado: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 117.165430000000000000
          Top = 15.118120000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'id_empregado'
          DataSet = RLDS_Empregados
          DataSetName = 'frxDBDatasetEmpregados'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetEmpregados."id_empregado"]')
        end
        object frxDBDatasetEmpregadoscod_departamento: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 362.834880000000000000
          Top = 15.118120000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'cod_departamento'
          DataSet = RLDS_Empregados
          DataSetName = 'frxDBDatasetEmpregados'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetEmpregados."cod_departamento"]')
        end
        object frxDBDatasetEmpregadosdata_admissao: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 585.827150000000000000
          Top = 15.118120000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'data_admissao'
          DataSet = RLDS_Empregados
          DataSetName = 'frxDBDatasetEmpregados'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetEmpregados."data_admissao"]')
        end
        object frxDBDatasetEmpregadosnm_empregado: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 56.692950000000000000
          Top = 56.692950000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'nm_empregado'
          DataSet = RLDS_Empregados
          DataSetName = 'frxDBDatasetEmpregados'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetEmpregados."nm_empregado"]')
        end
        object frxDBDatasetEmpregadosnm_funcao: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 249.448980000000000000
          Top = 56.692950000000000000
          Width = 170.078850000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'nm_funcao'
          DataSet = RLDS_Empregados
          DataSetName = 'frxDBDatasetEmpregados'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetEmpregados."nm_funcao"]')
        end
        object frxDBDatasetEmpregadossalario: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 468.661720000000000000
          Top = 56.692950000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'salario'
          DataSet = RLDS_Empregados
          DataSetName = 'frxDBDatasetEmpregados'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetEmpregados."salario"]')
        end
        object frxDBDatasetEmpregadoscomissao: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 627.401980000000000000
          Top = 56.692950000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          DataField = 'comissao'
          DataSet = RLDS_Empregados
          DataSetName = 'frxDBDatasetEmpregados'
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetEmpregados."comissao"]')
        end
      end
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 79.370124820000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo6: TfrxMemoView
          AllowVectorExport = True
          Left = 162.519790000000000000
          Top = 18.897650000000000000
          Width = 408.189235380000000000
          Height = 45.354354820000000000
          ContentScaleOptions.Constraints.MaxIterationValue = 0
          ContentScaleOptions.Constraints.MinIterationValue = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -35
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'Relat'#243'rio de Empregados')
          ParentFont = False
        end
      end
    end
  end
end
